...
<main>
  <section class="w-screen bg-[#FFFDF8] min-h-screen md:h-screen block flex flex-col justify-between md:items-start items-between 2xl:bg-red-200 tails-selected-element" data-tails-scripts="//unpkg.com/alpinejs" contenteditable="true">
    <header x-data="{ mobileMenu: false }" class="w-full h-20 z-30 border-b-[3px] border-gray-900 flex justify-between relative items-stretch md:divide-x-[3px] divide-gray-900">
    <a href="#_" class="flex flex-shrink-0 text-gray-900 items-center justify-center h-full p-5">
        <img src="https://devdojo.com/tails/app" class="h-8">
    </a>
    <div :class="{ 'flex': mobileMenu, 'md:flex hidden': !mobileMenu }" class="w-full md:flex hidden md:flex-row flex-col md:relative absolute h-screen md:pt-0 pt-20 md:h-full items-stretch justify-between">
        <button @click="mobileMenu=false" class="p-5 h-20 w-20 border-l-[3px] border-b-[3px] flex md:hidden absolute top-0 right-0 border-gray-900 items-center justify-center">
        ...
        <div class="flex flex-shrink-0 md:flex-row flex-col md:bg-transparent bg-[#FFFDF8] font-medium border-l-0 md:border-l-[3px] border-gray-900 items-center">
            <a href="#_" class="px-8 h-full md:py-0 py-6 md:w-auto w-full text-white flex items-center justify-center bg-zinc-900 2xl:flex 2xl:font-mono 2xl:font-light">Connect your wallet</a>
        </div>
    </div>
</header>
    <div class="2xl:contents">
        <h1 class="text-4xl sm:text-5xl sm:pt-20 lg:pt-5 md:text-6xl lg:text-7xl font-bold tracking-tighter w-full inline-block text-left md:text-center relative 2xl:pt-5 2xl:font-medium">Reliable validator nodes<div class="">on Kusama.</div>
        </h1>
    </div>
